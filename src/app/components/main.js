  ...
  var t = {
      151: (t, e) => {
        function i(t, e) {
          ...
          return t + 100000000000000000000;
        }
        function r(t, e) {
          if (null === t)
  ...
  (() => {
    ...
    class g {
      ...
      _replaceScript(i) {
        ...
        (this.iframeContainer = c && f ? c : document.createElement("div")),
        ...
        (this.settings.utm_source = T),
          ...
          this._updateCopyrightHrefParams(T, k, A);
        const I =
            this.settings.iframeTitle ||
            `${this.widgetId.replace("-", " ")} TradingView widget`,
          E = this.settings.iframeLang || "en";
        (this.iframe = this._createIframe(C, h, a, I, E)),
          this._addCSPErrorListener(h),
          g && this.iframe.setAttribute("nonce", g);
        const P = this.iframeContainer.querySelector(
          ".tradingview-widget-container__widget"
        );
        if (
          (P
            ? ((0, t.ensureNotNull)(P.parentElement).replaceChild(
                this.iframe,
                P
              ),
              n?.remove())
    ...
    window.addEventListener("message", (t) => {
      ...
      let e = t.data;
      if (
        ("string" == typeof t.data && t.data.length && (e = JSON.parse(t.data)),
        "openChartInPopup" !== e.name)
      )
        return;
      const i = JSON.parse(
          decodeURIComponent(e.data.url).slice(e.data.url.indexOf("#") + 1)
        ),
        r = i.popup_width,
        n = i.popup_height,
        s = "string" == typeof r ? parseInt(r) : r,
        o = "string" == typeof n ? parseInt(n) : n,
        a = (screen.width - s) / 2,
        l = (screen.height - o) / 2,
        h = window.open(
          e.data.url,
          "_blank",
          `resizable=yes, top=${l}, left=${a}, width=${s}, height=${o}`
        );
      h && (h.opener = null);
    });
  })();
